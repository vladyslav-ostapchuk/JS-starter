...
function t5() {
    a5_res = a5.filter(elem => {
        return typeof (elem) === 'number' && elem % 2 === 0
    })
    return a5_res
}
...
function t6() {
    a6_res = a6.filter(elem => {
        return typeof (elem) === 'number' && elem > 14
    })
    return a6_res
}
...
function t12() {
    // if (Array.isArray(a12)) { return true } else { return false }
...
function t14() {
    // return a14_sym.includes(a14_sym.toLowerCase()) || a14.includes(a14_sym.toUpperCase())
    let sL = []
    a14.map(elem => {
        sL.push(elem.toLowerCase())
    })
    let bL = a14_sym.toLowerCase()
    return sL.includes(bL)
